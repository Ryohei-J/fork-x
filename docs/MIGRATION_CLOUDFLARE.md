...
- [x] **末尾スラッシュ・ルーティングの挙動差**: `https://forkx.dev/disclaimer`・`/disclaimer/`ともに正常表示を確認済み。
...
- [x] **CloudflareダッシュボードのBuild/Deployコマンド設定**: `pnpm build` + `wrangler deploy`に変更済み。`https://forkx.dev`へのデプロイ成功を確認済み。
...
3. スマホ表示の確認（docs/TODO.mdに既存の未対応タスクとしても記載あり、移行のついでに崩れがないかは見ておく）
